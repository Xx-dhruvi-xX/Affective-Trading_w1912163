...
const STOCKS = [
  { symbol: 'AAPL', name: 'Apple Inc.', seed: 187.5 },
  { symbol: 'GOOGL', name: 'Alphabet Inc.', seed: 1250.0 },
  { symbol: 'AMZN', name: 'Amazon.com Inc.', seed: 1900.0 },
  { symbol: 'MSFT', name: 'Microsoft Corporation', seed: 150.0 },
  { symbol: 'TSLA', name: 'Tesla Inc.', seed: 300.0 },
  { symbol: 'META', name: 'Meta Platforms Inc.', seed: 250.0 },
]

const nextPrice = p => Math.max(1, parseFloat((p * (1 + (Math.random() - 0.5) * 0.016)).toFixed(2)));
...
export default function TradingSimulator(){
  ...
  const[prices, setPrices] = useState(() => Object.fromEntries(STOCKS.map(s => [s.symbol, s.seed])));
  const[prevPrices, setPrevPrices] = useState(() => Object.fromEntries(STOCKS.map(s => [s.symbol, s.seed])));
  ...
  const holdingsValue = Object.entries(holdings).reduce((s, [sym, q]) => s + q * (prices[sym] ?? 0),0);
  ...
  const executeTrade = useCallback(async() => {
    ...
    if (!qty || qty <= 0) {
      setMsg({ok: false, text: 'Quantity must be atleast 1.'}); 
      return;
    }
    if(side === 'BUY'){
      if(price * qty > cash){
        setMsg({ ok: false, text: `Insuffcient funds. Need ${fmtCcy(price * qty)}.`});
        return;
      }
      ...
      if(qty > owned){
        setMsg({ ok: false, text: `you only hold ${owned} shares(s) of ${selected}.`});
        return;
      }
      setCash(c => parseFloat((c + price * qty).toFixed(2)));
      setHoldings(h =>{
        const u = {...h};
        const left = (u[selected] ?? 0) - qty;
        if(left === 0) delete u[selected];
        else u[selected] = left;
        return u;
      });
    }
    const trade = {id: Date.now(), symbol: selected, side, quantity: qty, price, time: new Date().toLocaleString('en-GB')};
    setTrades(t => [trade, ...t]);
    setMsg({ok: true, text: `${side} ${qty} x ${selected} @ ${fmtCcy(price)}`});
    if(sessionId){
      try{
        const r = await fetch( 
          `${FLASK_BASE}/sessions/${sessionId}/trades`,{
            method: 'POST',
            headers: {'Content-Type':'application/json'},
            body: JSON.stringify({symbol: selected, side, quantity: qty, price}),
          }
        );
        setBackendOk(r.ok);
      } catch{
    ...
    return(
      <div className="sim">
        ...
        <div className="sim_topbar-right">
          ...
          <button className="sim_end-btn" onClick={handleFinish}>End Session</button>
          </div>
          </div>
          <div className="sim_body">
            <div className="sim_col">
              <p className="sim_col-heading">Market</p>
        ...
        <div className="sim_trade-col">
          ...
          <div className="sim_cost-row">
            ...
            <span className={`sim_cost-value sim_cost-value--${side.toLowerCase()}`}>{fmtCcy(tradeValue)}</span>
          </div>

          <button className={`sim_submit sim_submit--${side.toLowerCase()}`} onClick={executeTrade}>
          ...
          <div className="sim_history">
            <p className="sim_col-heading" style={{ padding: 0, marginBottom: 12 }}>Trade History</p>
            {trades.length === 0 ? (
              ...
              <div className="sim_history-list">
                {trades.map(t => (
                  <div key={t.id} className="sim_history-row">
                    <span className={`sim_history-badge sim_history-badge--${t.side.toLowerCase()}`}>{t.side}</span>
        ...
        <div className="sim_col" style={{ padding: 20 }}>
          <p className="sim_col-heading" style={{ padding: 0, marginBottom: 16 }}>Portfolio</p>
          ...
          <div className="sim_port-total">
            ...
            <p className={`sim_port-total-value sim_port-total-value--${pnl >= 0 ? 'gain' : 'loss'}`}>
              {fmtCcy(totalValue)}
            </p>
            <p className={`sim_port-total-pnl sim_port-total-pnl--${pnl >= 0 ? 'gain' : 'loss'}`}>
              {pnl >= 0 ? '+' : ''}{fmtCcy(pnl)} ({pnlPct.toFixed(2)}%)
            </p>
          </div>
            ...
            Object.entries(holdings).map(([sym, qty]) => (
              <div key={sym} className="sim_holding-row">
                ...
                <div>
                  ...
                  <p className="sim_holding-price">{fmtCcy(prices[sym])}/sh</p>
                </div>
              </div>
            ))
          )}
